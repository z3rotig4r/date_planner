...
CREATE TABLE IF NOT EXISTS public.link_previews (
  ...
  image TEXT,
  site_name TEXT,
  type TEXT, -- 'og' | 'naver' | 'youtube' | 'fallback'
  raw JSONB, -- 확장성을 위한 원본 데이터 저장
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);
...
-- [Link Preview 정책]
DROP POLICY IF EXISTS "Anyone can read link cache" ON link_previews;
DROP POLICY IF EXISTS "Anyone can insert link cache" ON link_previews;
CREATE POLICY "Anyone can read link cache" ON public.link_previews FOR SELECT USING (true);
CREATE POLICY "Anyone can insert link cache" ON public.link_previews FOR INSERT WITH CHECK (true);
